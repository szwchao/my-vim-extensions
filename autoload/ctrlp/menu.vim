...
function! ctrlp#menu#init()
  " 移除menu本身
  let ctrlp_ext_snames = map(copy(g:ctrlp_ext_vars), 'v:val.sname')
  let idx = index(ctrlp_ext_snames, 'menu')
  let ctrlp_ext_snames_without_menu = copy(g:ctrlp_ext_vars)
  call remove(ctrlp_ext_snames_without_menu, idx)
  return map(copy(s:builtins) + copy(ctrlp_ext_snames_without_menu),
...
func! ctrlp#menu#accept(mode, str)
  ...
  if !empty(target)
    " 将accept换成id，形成ctrlp#xxx#id()，但有的扩展用的ctrlp#acceptfile，替换不了，先用ctrlp#lastcommit#exit()代替
    "call ctrlp#init(call(substitute(target.accept, '#accept', '#id', ''),[]))
    call ctrlp#init(call(substitute(target.exit, '#exit()', '#id', ''),[]))
  endif
endfunc
